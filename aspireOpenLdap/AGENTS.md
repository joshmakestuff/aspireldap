...
Change notifications: `WithChangeNotifications(string checkpoint = "1 1", int sessionLog = 100)` loads the `syncprov` overlay, making the server an RFC 4533 sync provider (equivalent to `WithOverlay(OpenLdapOverlay.SyncProv(...))` — call at most one of the two). Point any RFC 4533 client at the resource's endpoint; the CLI baseline is `ldapsearch -E sync=rp`. **Server-side enablement only — there is no first-party subscribe API**: `System.DirectoryServices.Protocols` cannot implement RFC 4533 (per-entry SyncState controls are dropped on every platform; the persist stage never starts on Linux), and no second LDAP client stack is bundled. The checkpoint is `"<ops> <minutes>"`, both positive integers, validated at the call (slapd rejects zero minutes at bootstrap with exit 80); the defaults are checkpoint `1 1` (keeps `contextCSN` durable across unclean stops) and session log 100 (gives delta deletes on cookie resume).
...
When `EnableRestApi` is true, LdapAdmin exposes an unauthenticated JSON API under `/api/v1` using
the same AppHost-provided admin bind as the UI. It covers directory metadata/children, search,
entry CRUD/rename/password, schema, and LDIF export/plan/import; failures use RFC 9457 problem
details. It is a local-development surface: no authentication or CORS is added, so do not expose it.
