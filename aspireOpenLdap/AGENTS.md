...
Change notifications: `WithChangeNotifications(string checkpoint = "1 1", int sessionLog = 100)` loads the `syncprov` overlay, making the server an RFC 4533 sync provider (equivalent to `WithOverlay(OpenLdapOverlay.SyncProv(...))` — call at most one of the two). Point any RFC 4533 client at the resource's endpoint; the verified CLI baseline is `ldapsearch -E sync=rp`. **Server-side enablement only — there is deliberately no first-party subscribe API**: `System.DirectoryServices.Protocols` cannot implement RFC 4533 (per-entry SyncState controls are dropped on every platform; the persist stage never starts on Linux), and a second LDAP client stack was rejected — decided in [aspireldap#88](https://github.com/joshmakestuff/aspireldap/issues/88). The checkpoint is `"<ops> <minutes>"`, both positive integers, validated at the call (slapd rejects zero minutes at bootstrap with exit 80); the defaults are the measured dev-right values (checkpoint `1 1` keeps `contextCSN` durable across unclean stops; session log 100 gives delta deletes on cookie resume).
