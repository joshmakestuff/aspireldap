...
public class BootstrapRegressionTests : IDisposable
{
    ...
    public async Task Stale_Alias_File_Is_Ignored_When_Canonical_Is_Set()
    {
        ...
        // fail-closed.
        var selected = await DockerCli.RunAsync(cts.Token,
            "run", "--rm",
            "-e", "LDAP_ACCESSLOG_PASSWORD_FILE=/run/secrets/obsolete-missing",
            "--entrypoint", "bash", image, "-c", resolve);
    ...
    public async Task File_Secret_Feeds_The_Admin_Password_End_To_End()
    {
        ...
        await DockerCli.WaitForLdapReadyAsync(name, "cn=admin,dc=example,dc=org", secretPassword, cts.Token);
    ...
    public async Task Prehashed_Default_Tree_Password_Passes_Through()
    {
        ...
        var hashRun = await DockerCli.RunAsync(cts.Token,
    ...
    public async Task Custom_Ldifs_Load_And_Suppress_The_Default_Tree()
    {
        ...
        try
        {
            await File.WriteAllTextAsync(Path.Combine(ldifDir, "10-custom.ldif"),
                ...
                "ou: custom\n", cts.Token);
            DockerCli.WidenPermissionsForContainer(ldifDir);
    ...
    public async Task Completed_Volume_Restart_Preserves_Data()
    {
        ...
        var add = await DockerCli.RunAsync(cts.Token,
            "exec", name, "bash", "-c",
        ...
        _ = await DockerCli.RunAsync(cts.Token, "stop", name);
        _ = await DockerCli.RunAsync(cts.Token, "start", name);
        await DockerCli.WaitForLdapReadyAsync(name, "cn=admin,dc=example,dc=org", AdminPassword, cts.Token);
    ...
    public async Task Escaped_Semicolon_Root_Bootstraps()
    {
        ...
        var run = await DockerCli.RunAsync(cts.Token,
            ...
            image);
        Assert.True(run.ExitCode == 0, $"docker run failed: {run.Output}");
        await DockerCli.WaitForLdapReadyAsync(name, $"cn=admin,{root}", AdminPassword, cts.Token);
    ...
    public async Task Custom_Ldif_Continue_On_Error_Skips_Rejects_But_Default_Fails_Loud()
    {
        ...
        try
        {
            ...
            var strict = await DockerCli.RunAsync(cts.Token,
                "run", "--rm",
                "-e", $"LDAP_ADMIN_PASSWORD={AdminPassword}",
                "-v", $"{ldifDir}:/ldifs:ro",
                image);
            ...
            var name = NewContainer();
            var lenient = await DockerCli.RunAsync(cts.Token,
                "run", "-d", "--name", name,
                "-e", $"LDAP_ADMIN_PASSWORD={AdminPassword}",
