...
// connection string. The admin is referenced here as an ordinary project resource.
...
var ldap = builder.AddOpenLdap("openldap")
    ...
    // The sizelimit witness: a non-root account that CAN delete, but only under
    ...
    // The large-scale container: far past the tree's Cap of 48, so the rail's
    // "search this container" row renders against a real directory.
    ...
    // size=10 keeps the sweep scenario cheap: ~25 children under ou=bulk-del already exceed
