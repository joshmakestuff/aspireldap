...
# Aspire health-check probe connections are dropped from the stats log by default
# (see probe_log_filter.sh); set to yes to log them like any other connection.
export LDAP_LOG_HEALTH_PROBES="${LDAP_LOG_HEALTH_PROBES:-no}"
...
########################
# Apply opt-in access-control rules (olcAccess modify on the mdb database) generated by the
# host WithAccessControl(...) API, mounted at /access.ldif. Applied ONLINE after the data load
# so referenced principal DNs (e.g. a seeded service account) already exist. cn=config changes
# need the EXTERNAL (root) identity, so this is a separate step from the admin-bound data load.
########################
ldap_apply_access() {
    [[ -s /access.ldif ]] || return 0
    info "Applying LDAP access rules from /access.ldif"
    debug_execute ldapmodify -Y EXTERNAL -H "ldapi:///" -f /access.ldif
}
...
ldap_initialize() {
    ...
    if ! is_dir_empty "$LDAP_DATA_DIR"; then
        ...
        # Access-control rules — after the data load so principal DNs they reference exist.
        ldap_apply_access
        # Enable TLS
        if is_boolean_yes "$LDAP_ENABLE_TLS"; then
            ldap_configure_tls
