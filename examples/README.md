...
- **Fake data** — `WithFakeDirectory(seed: ...)` seeds deterministic (fixed-seed) `inetOrgPerson`/`groupOfNames` entries, generated by the bundled [`LdifDotNet.Generator`](https://www.nuget.org/packages/LdifDotNet.Generator).
