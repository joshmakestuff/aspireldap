...
- **DN-valued attributes point at real entries.** `member`, `owner`, `seeAlso`, `manager`, and `secretary` are generated as DNs that `slapadd` accepts *and* that resolve to entries the generator produced, so group traversal and `memberOf` behaviour can actually be exercised. Version behavior: 0.6.0 emitted lorem free text such as `member: consequuntur cupiditate`, which `slapadd` rejects; 0.7.0 made the values valid but pointed every one at the entry's own parent DN ([ldifdotnet#68](https://github.com/joshmakestuff/ldifdotnet/issues/68)); 0.8.0 makes them resolve to generated entries.

  Sources, in order: the attribute's `options.DnPool[...]`, then DNs the generator has already minted, then the parent DN. The last is reached only before anything has been minted — in the example above, the first generated entry's `secretary` is the one value that points at the parent DN. Values are multi-valued where the schema allows (`options.MaxDnValues`, default 4; a `SINGLE-VALUE` attribute always gets one).
