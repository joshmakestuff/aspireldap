...
| **REST transport** (`Category=RestIntegration`) | Starts a fresh LdapAdmin AppHost in a separate test process and calls `/api/v1` over HTTP. | Opt-in mapping, stable JSON, RFC 9457 failures, entry mutations, schema, and LDIF transport. Its separate process avoids a second Aspire testing fixture sharing process-wide orchestration state. | The direct service layer and packaged payload boundary, covered by the neighboring tiers. |
...
```bash
dotnet test AspireOpenLdap.slnx -c Release --filter "Category!=Integration&Category!=RestIntegration&Category!=CleanConsumer" \
...
- **`Category!=Integration&Category!=RestIntegration&Category!=CleanConsumer` is enforced in the config.** A mutation
